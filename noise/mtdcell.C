...
void mtdcell(){
	...
	for (int ifile=0;ifile<2;ifile++){
		if (ifile==0){ f = new TFile("/star/institutions/rice/llope/noise/noise_14112036.root","read");	} else
		if (ifile==1){ f = new TFile("/star/institutions/rice/llope/noise/noise_14112.root","read");	}
		for (int ibl=0;ibl<30;ibl++){
			for (int i=0;i<5;i++){
	...
	TH2D *hrat2D	= new TH2D("hrat2D","Run14112036 / Run14112011 by (BL,pos)",15,0.5,15.5,5,0.5,5.5);
	for (int ibl=0;ibl<30;ibl++){
		for (int i=0;i<5;i++){
...
//  	gStyle->SetPadLeftMargin(0.09);	
	...
	ccan[ican]->Print("mtdcell.ps]");
	gSystem->Exec("/usr/bin/ps2pdf ./mtdcell.ps ./mtdcell_14112036.pdf");
	gSystem->Exec("/bin/cp ./mtdcell_14112036.pdf /star/u/llope/WWW/files/");
