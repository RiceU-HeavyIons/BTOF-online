...
struct TOFData{
  Double_t triggerTime[4];
  Int_t nTofHits;
  Int_t flag[100000];
  Int_t tray[100000];
  Int_t channel[100000];
  Double_t tdc[100000];
};
TOFData mCellData;
...
int main(int argc, char *argv[])
{
 int nEvts = 100;
  ...
  TTree *tree = new TTree("raw","raw hits");
  tree->SetAutoSave(100000);
  tree->Branch("triggerTime",mCellData.triggerTime,"triggerTime[4]/D");
  tree->Branch("nTofHits",&mCellData.nTofHits,"nTofHits/I");
  tree->Branch("flag",mCellData.flag,"flag[nTofHits]/I");
  tree->Branch("tray",mCellData.tray,"tray[nTofHits]/I");
  tree->Branch("channel",mCellData.channel,"channel[nTofHits]/I");
  tree->Branch("tdc",mCellData.tdc,"tdc[nTofHits]/D");

  TH2D *leTrg = new TH2D("leTrg","leTrg",124,0,124,4300,0,4300);
  TH2D *TrTrg = new TH2D("TrTrg","TrTrg",124,0,124,4300,0,4300);
  TH2D *leTrg0 = new TH2D("leTrg0","leTrg0",124,0,124,4300,0,4300);
  TH2D *TrTrg0 = new TH2D("TrTrg0","TrTrg0",124,0,124,4300,0,4300);
  TH2D *leTrg1 = new TH2D("leTrg1","leTrg1",124,0,124,4300,0,4300);
  TH2D *TrTrg1 = new TH2D("TrTrg1","TrTrg1",124,0,124,4300,0,4300);
  TH2D *leTrg2 = new TH2D("leTrg2","leTrg2",124,0,124,4300,0,4300);
  TH2D *TrTrg2 = new TH2D("TrTrg2","TrTrg2",124,0,124,4300,0,4300);
  TH2D *leTrg3 = new TH2D("leTrg3","leTrg3",124,0,124,4300,0,4300);
  TH2D *TrTrg3 = new TH2D("TrTrg3","TrTrg3",124,0,124,4300,0,4300);
  TH2D *leTrg121 = new TH2D("leTrg121","leTrg121",200,0,200,4300,0,4300);
  TH2D *TrTrg121 = new TH2D("TrTrg121","TrTrg121",200,0,200,4300,0,4300);
  ...
  printf("Start run!\n");
	...
	while((c = getopt(argc, argv, "D:d:m:o:h")) != EOF) {
		switch(c) {
		case 'd' :
			rtsLogLevel(optarg) ;
			break ;
        ...
        TFile *fout = new TFile(out_fname,"recreate");
//	printf("creating OutputTof.root");
				...
				while(dd->iterate()) {
					...
					//	printf("TOF RDO %d: words %d:\n",r+1,tof->ddl_words[r]) ;
						...
						//	printf("\t%d: 0x%08X [%u dec]\n",i,tof->ddl[r][i],tof->ddl[r][i]) ;
			                          ...
			                          unsigned int dataword = tof->ddl[r][i];
			                          ...
			                          if( (dataword&0xF0000000)>>28 == 0x2) {   /// trigger time here.
                        			        if(mCellData.triggerTime[r]==0)
                                			{
			                                  mCellData.triggerTime[r] = 25.*(dataword & 0xfff);
                        			          //printf(" r=%d triggerTime=0x%08X\n",r,dataword);
			                                }
                        			        continue;
			                          }
                        			  ...
                        			  ///
			                          mCellData.flag[nTof] = (edgeid==4) ? r+1 : -(r+1);
			                          mCellData.tray[nTof] = trayid;
			                          mCellData.channel[nTof] = tdcchan + (tdcid%4)*8+tdigid*24+halftrayid*96;
			                          unsigned int timeinbin = ((dataword&0x7ffff)<<2)+((dataword>>19)&0x03);
			                          mCellData.tdc[nTof] = timeinbin * 25./1024;
			                          nTof++;

			                          double tDiff = timeinbin * 25./1024 - mCellData.triggerTime[r];
			                          while(tDiff<0.) tDiff += 51200.;
			                          if(edgeid==4){
								 leTrg->Fill(trayid-1, tDiff);
                                                                 if(trayid==121) leTrg121->Fill(mCellData.channel[nTof], tDiff);
                                                                 if(r==0) leTrg0->Fill(trayid-1, tDiff);
                                                                 if(r==1) leTrg1->Fill(trayid-1, tDiff);
                                                                 if(r==2) leTrg2->Fill(trayid-1, tDiff);
                                                                 if(r==3) leTrg3->Fill(trayid-1, tDiff);

						  }
			                          else{
								 TrTrg->Fill(trayid-1, tDiff);
                                                                 if(trayid==121) TrTrg121->Fill(mCellData.channel[nTof], tDiff);
                                                                 if(r==0) TrTrg0->Fill(trayid-1, tDiff);
                                                                 if(r==1) TrTrg1->Fill(trayid-1, tDiff);
                                                                 if(r==2) TrTrg2->Fill(trayid-1, tDiff);
                                                                 if(r==3) TrTrg3->Fill(trayid-1, tDiff);
						  }
		                  ...
		                  mCellData.nTofHits = nTof;

                		  printf("There are %d Tof Hits in this event\n",nTof);
