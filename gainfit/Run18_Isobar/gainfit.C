...
void gainfit() {
	...
	TString filename1 = "online_19071027.root"; // A
	TString filename2 = "online_19071025.root"; // B
	TString filename3 = "online_19071026.root"; // C
	...
	f1->Close();
	TFile *f1	= new TFile(filename2,"READ");
		f1->cd();
		TH1D *hadc = (TH1D*)gDirectory->Get(hstringadc);
		      hadc->SetName("hadc");
		TH1D *htot = (TH1D*)gDirectory->Get(hstringtot);
	...
	f1->Close();
	TFile *f1	= new TFile(filename3,"READ");
		f1->cd();
		TH1D *hadc = (TH1D*)gDirectory->Get(hstringadc);
		      hadc->SetName("hadc");
		TH1D *htot = (TH1D*)gDirectory->Get(hstringtot);
