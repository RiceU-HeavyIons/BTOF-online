...
//sprintf(type,"13080080_tof");
sprintf(type,"13081013_tof");
...
cout<<buf<<endl;
	...
	TF1 *g1	= new TF1("g1","gaus",-770,-730);
		g1->SetLineWidth(2);
		g1->SetLineColor(2);
	TF1 *g2	= new TF1("g2","gaus",725,765);
		g2->SetLineWidth(2);
		g2->SetLineColor(2);
	...
	float crossingperiod	= (gpars2[1]-gpars1[1])/14.;
	...
	for (int i=0;i<6;i++){
		...
		if (i< 4){ 
			...
			htrgdtime[i]->GetXaxis()->SetRangeUser(-100.,5100.);
		} else if (i==4){ 
			htrgdtime[i]->SetTitle(Form("trgdtime, VPD 121")); 
			htrgdtime[i]->GetXaxis()->SetRangeUser(-100.,600.);
		} else if (i==5){ 
	...
	for (int ithub=0;ithub<6;ithub++){
		for (int ih=0;ih<2;ih++){
			...
			tbo[0][ithub][ih]->DrawLatex(0.22,0.8,Form("%3.0f",xmax1));
			if (ymax>0){
				float xmax2	= hbunchidoffset_thubhalf[ithub][ih]->GetBinCenter(ibmax2);
				tbo[1][ithub][ih]->DrawLatex(0.22,0.72,Form("%3.0f",xmax2));
			}
			//
...
// 	ccan[ican]->cd(); ccan[ican]->Divide(8,6,0.0001,0.0001);	//!!!!!
 	ccan[ican]->cd(); ccan[ican]->Divide(6,3,0.0001,0.0001);	//!!!!!
...
// 	ccan[ican]->Print(psfileopen);
	...
	ccan[ican]->cd(1);
		gPad->SetLogz(1);
		htrgdtime_tray->GetYaxis()->SetRangeUser(-100.,5100.);
...
//	cout<<buf<<endl;
	...
	ccan[ican]->cd(1);
		gPad->SetLogz(1);
		//htrgdtime_trayoff->GetYaxis()->SetRangeUser(500.,1100.);
